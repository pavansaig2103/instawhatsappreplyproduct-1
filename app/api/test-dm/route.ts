...
export async function POST(request: Request) {
  ...
  const instagramHandle = body.instagramHandle?.trim() || `@${business.slug}_${channel.toLowerCase()}_test_lead`;
  const fallbackName = body.contactName?.trim() || "New Lead";

  if (isDevelopment) {
    console.log("[Test DM] Incoming message", inboundText);
    console.log("[Test DM] Selected business", {
      id: business.id,
      name: business.name,
      type: business.businessType
    });
    console.log("[Test DM] Selected channel", channel);
  }
  ...
  let nextLeadState = conversation.leadState;
  ...
  if (isDevelopment) {
    console.log("[Test DM] FAQ matched", Boolean(faqAnswer));
  }
  ...
  if (isGreeting(inboundText) && !waitingForName && conversation.leadState !== LeadCaptureState.AWAITING_PHONE) {
    ...
    if (isDevelopment) {
      console.log("[Test DM] Calling Groq AI fallback");
      console.log("[Test DM] GROQ_API_KEY exists", Boolean(process.env.GROQ_API_KEY));
    }
    ...
    if (isDevelopment) {
      console.log("[Test DM] Parsed AI answer and confidence", {
        answer: fallback.answer,
        confidence: fallback.confidence
      });
      if (fallback.error) {
        console.log("[Test DM] Groq error", fallback.error);
      }
    }
    ...
    reply = isLeadCaptureTrigger(inboundText)
      ? `${faqAnswer}\n\n${ASK_NAME_REPLY}`
      : faqAnswer;
    replyType = ReplyType.FAQ_MATCH;
    leadUpdate.intent = inferIntent(inboundText);
